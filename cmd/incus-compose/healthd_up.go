	...
	return &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Sources: cli.EnvVars("INCUS_COMPOSE_HEALTHD_UP_IMAGE"),
			},
			&cli.StringFlag{
				Name:    "binary",
				Usage:   "Path to local ic-healthd binary (uses images:alpine/edge instead of OCI image)",
				Sources: cli.EnvVars("INCUS_COMPOSE_HEALTHD_UP_BINARY"),
			},
			&cli.StringFlag{
				Name:    "incus",
				...
				Sources: cli.EnvVars("INCUS_COMPOSE_HEALTHD_UP_INCUS"),
			},
			&cli.StringFlag{
				Name:    "network",
				Usage:   "Incus bridge for healthd to use (default: auto-detect)",
				Sources: cli.EnvVars("INCUS_COMPOSE_HEALTHD_UP_NETWORK"),
			},
			&cli.StringFlag{
				Name:    "pull",
				Usage:   `Pull image before running ("always"|"missing"|"never"|"policy")`,
				Value:   "policy",
				Sources: cli.EnvVars("INCUS_COMPOSE_HEALTHD_UP_PULL"),
			},
			&cli.DurationFlag{
				Name:    "timeout",
				Usage:   "Timeout for stopping",
				Value:   10 * time.Second,
				Sources: cli.EnvVars("INCUS_COMPOSE_HEALTHD_UP_TIMEOUT"),
			},
		},
