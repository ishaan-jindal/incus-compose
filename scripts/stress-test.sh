...
simplenginx() {
  ...
  INCUS_PROJECT="${project_name}" incus profile device add default root disk path=/ pool=default

  INCUS_PROJECT="${project_name}" incus network create "${network_name}"
  INCUS_PROJECT="${project_name}" incus profile device add default eth0 nic network="${network_name}"

  INCUS_PROJECT="${project_name}" incus launch docker.io:library/nginx:alpine web

  INCUS_PROJECT="${project_name}" incus stop web
  INCUS_PROJECT="${project_name}" incus remove web

  echo "yes" | incus project rm -f "${project_name}"
  incus network rm "${network_name}"
}
...
twoservices() {
  ...
  INCUS_PROJECT="${project_name}" incus launch docker.io:nginxinc/nginx-unprivileged:alpine ic-healthd
  INCUS_PROJECT="${project_name}" incus launch docker.io:nginxinc/nginx-unprivileged:alpine web1
  INCUS_PROJECT="${project_name}" incus launch docker.io:nginxinc/nginx-unprivileged:alpine web2

  INCUS_PROJECT="${project_name}" incus stop ic-healthd
  INCUS_PROJECT="${project_name}" incus stop web1
  INCUS_PROJECT="${project_name}" incus stop web2

  INCUS_PROJECT="${project_name}" incus remove ic-healthd
  INCUS_PROJECT="${project_name}" incus remove web1
  INCUS_PROJECT="${project_name}" incus remove web2
...
run_timed() {
  local label=$1 func=$2 project=$3 network=$4
  local start end duration status

  start=$(date +%s.%N)
  if "$func" "$project" "$network" >"${RESULTS_DIR}/${project}.log" 2>&1; then
    status="ok"
  else
    status="FAIL"
  fi
...
summarize_batch() {
  ...
  local -a times=()
  for f in "${RESULTS_DIR}/b${batch}-${label}"*.time; do
    [[ -e "$f" ]] || continue
    times+=("$(cat "$f")")
  done
...
for batch in $(seq 1 "${BATCHES}"); do
  ...
  for ((i = 1; i <= PAIRS; i++)); do
    run_timed "simplenginx" simplenginx "b${batch}-simplenginx${i}" "b${batch}-simple${i}" &
    pids+=("$!")

    run_timed "twoservices" twoservices "b${batch}-twoservices${i}" "b${batch}-two${i}" &
    pids+=("$!")
  done
